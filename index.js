...
function compute() {
  ...
  const input = document.getElementById('hexcode').value;
  ...
  if (res.result.loss > maxLoss) {
    res.lossMsg = 'Not good yet...';
    window.setTimeout(function() { compute() }, 10)
  } else {
    res.lossMsg = 'It\'s Perfect!';
    setRunning(false)
  }
  ...
  const textRGB = document.getElementById('textRGB');
  const textFILTER = document.getElementById('textFILTER');
  const testLOSS = document.getElementById('textLOSS');
  const rgbColor = res.color.toRgb();
  const lossValue = res.result.loss.toFixed(2);
  
  pixelHEX.style.backgroundColor = rgbColor;
  textRGB.innerText = rgbColor;
  testLOSS.innerText = lossValue;
  lossMessage.innerHTML = res.lossMsg;
  
  pixelFILTER.style.filter = String(res.result.filterRaw);
  pixelFILTER.style.webkitFilter = String(res.result.filterRaw);
  textFILTER.innerText = res.result.filter;
}

function setRunning(isRunning) {
  const btnRun = document.getElementById('run');
  btnRun.innerText = isRunning ? "Running..." : "Run!";
...
function onStart() {
  document.addEventListener("DOMContentLoaded", function() {
    setRunning(false)
    document.getElementById('textFILTER').addEventListener("focus", (event) => {
